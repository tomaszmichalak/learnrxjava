...
public class ObservableSolutions extends ObservableExercises {
  ...
  /**
   * Given a stream of numbers, choose the even ones and return a stream like: <p> 2-Even 4-Even
   * 6-Even
   */
   ...
   * Flatten out all video in the stream of Movies into a stream of videoIDs
   *
   * Use flatMap this time instead of concatMap. In Observable streams it is almost always flatMap
   * that is wanted, not concatMap as flatMap uses merge instead of concat and allows multiple
   * concurrent streams whereas concat only does one at a time.
   *
   ...
   *
   * This is a variation of the problem we solved earlier, where we retrieved the url of the boxart
   * with a width of 150px. This time we'll use reduce() instead of filter() to retrieve the
   * _smallest_ box art in the boxarts list.
   *
   ...
   *
   * a -> "one", "two", "red", "blue" b -> "fish", "fish", "fish", "fish" output -> "one fish", "two
  ...
  /**
   * Don't modify any values in the stream but do handle the error and replace it with
   * "default-value".
   */
  ...
  /**
   * The data stream fails intermittently so return the stream with retry capability.
   */
